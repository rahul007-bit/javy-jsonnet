...
use wasmtime::{Config, Engine, Instance, Linker, Memory, Module, Store, WasmBacktraceDetails};
            ...
            Ok(output) => {
                ...
                match parsed_output {
                    Ok(parsed_output) => {
                        ...
                        response = response.status(parsed_output.status);
